...
class ProductController {
  ...
  async getProducts(req, res, next) {
    try {
      ...
      // Build cache key
      const cacheKey = `products:page:${page}:limit:${limit}:category:${category || 'all'}:search:${search || 'none'}:sort:${sortBy}:${sortOrder}`;
      ...
      // Cache the result
      await cacheHelper.set(cacheKey, response, 300); // Cache for 5 minutes
  ...
  async getProductById(req, res, next) {
    try {
      ...
      // Cache the product
      await cacheHelper.set(cacheKey, product, 3600); // Cache for 1 hour
